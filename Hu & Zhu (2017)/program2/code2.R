...
q2InfinityP=(sum(pk[Decided2,1])/sum(pk[,1]))/(media+(1-media)*(sum((sum(pk[Decided1,1])/sum(pk[,1]))+(sum(pk[Decided2,1])/sum(pk[,1])))))
# 根据式(24)得到稳态时观点为3的节点的加权比例
q3InfinityP=(media*(1-sum((sum(pk[Decided1,1])/sum(pk[,1]))+(sum(pk[Decided2,1])/sum(pk[,1])))))/(media+(1-media)*sum((sum(pk[Decided1,1])/sum(pk[,1]))+(sum(pk[Decided2,1])/sum(pk[,1]))))

# 根据式(17)得到稳态时观点为1的节点的比例
q1Infinity=(1-0.5)*(1-media)*q1InfinityP+0.3
# 根据式(17)得到稳态时观点为2的节点的比例
q2Infinity=(1-0.5)*(1-media)*q2InfinityP+0.2
# 根据式(25)得到稳态时观点为3的节点的比例
q3Infinity=(1-0.5)*(media+(1-media)*q3InfinityP)
