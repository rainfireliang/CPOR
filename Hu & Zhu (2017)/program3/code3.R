...
{   
  ...
  q2InfinityP[i]=(temp2+media[i]*(1-temp1-temp2))/(media[i]+(1-media[i])*(temp1+temp2))
  # 根据式(17)得到稳态时观点为1的节点的比例
  q1Infinity[i]=(1-0.5)*(1-media[i])*q1InfinityP[i]+0.3
  # 根据式(25)得到稳态时观点为2的节点的比例
  q2Infinity[i]=(1-0.5)*(media[i]+(1-media[i])*q2InfinityP[i])+0.2
}

# 可视化运行结果
# 数据准备。将输出数据转化为方便可视化的数据
data<-cbind(media,q1Infinity,q1InfinityP,q2Infinity,q2InfinityP)
data<-as.data.frame(data)
# 可视化结果
library(ggplot2)
linetypes=c(q1Infinity="solid",q1InfinityP="dashed",q2Infinity="dotted",q2InfinityP="dotdash")
ggplot(data,aes(x=media))+geom_line(aes(y=q1Infinity,linetype='q1Infinity'),size=1)+geom_line(aes(y=q1InfinityP,linetype="q1InfinityP"),size=1)+geom_line(aes(y=q2Infinity,linetype="q2Infinity"),size=1)+geom_line(aes(y=q2InfinityP,linetype="q2InfinityP"),size=1)+labs(x = "P", y = "qi, qiw")+scale_linetype_manual(values = linetypes,labels=c("q1","q1w","q2","q2w"))+labs(linetype='')+theme(axis.text = element_text(size = 15))+ theme(axis.title = element_text(size = 15))+theme(legend.text = element_text(size = 12))
